...
export default function ReservationsFlightTicketPage() {
  ...
  const handleSearch = () => {
    console.log('Searching flight ticket reservations with criteria:', {
      dateFrom,
      dateTo,
      sort,
      transactionType,
  ...
  const handleExport = async (type: 'copy' | 'excel' | 'pdf' | 'print') => {
    ...
    if (type === 'print') {
      printTable('#export-table', 'Reservations - Flight Ticket');
      return;
    }
    if (type === 'copy') {
    ...
    if (type === 'excel') {
      await exportTableToExcel('reservations-flight-ticket.xlsx', data);
      return;
    }
    await exportTableToPdf('reservations-flight-ticket.pdf', 'Reservations - Flight Ticket', data);
  };

  const ChevronIcon = ({ open }: { open: boolean }) => (
    <svg
      className={`w-5 h-5 text-gray-700 transition-transform ${open ? 'rotate-180' : ''}`}
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
    >
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-6">
          <h2 className="text-3xl font-light text-gray-800">Flight Ticket</h2>
          <div className="mt-2 text-sm text-gray-500">
            <a href="/dashboard" className="text-blue-600 hover:underline">
              Homepage
            </a>{' '}
            <span className="mx-2">»</span>
            <span className="text-gray-400">Flight Ticket</span>
          </div>
        </div>
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-100">
                      <tr>
                        ...
                        <th className="px-4 py-4 text-center text-base font-bold text-gray-900">Segment</th>
            ...
            <table id="export-table" className="w-full">
              <thead className="bg-gray-700 text-white">
                <tr>
                  ...
                  <th className="px-4 py-4 text-left font-semibold">Flight Date</th>
                  <th className="px-4 py-4 text-left font-semibold">Airline</th>
                  <th className="px-4 py-4 text-left font-semibold">Class</th>
                  <th className="px-4 py-4 text-left font-semibold">Route</th>
                  <th className="px-4 py-4 text-left font-semibold">Ticket Number</th>
              ...
              <tbody>
                {reservations.length === 0 ? (
                  <tr>
                    <td colSpan={13} className="px-6 py-10 text-center text-gray-700">
                  ...
                  reservations.map((row, idx) => (
                    <tr key={idx} className="border-t border-gray-200 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-700">{row.pnr}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-700">{row.flightDate}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.airline}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.class}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.route}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.ticketNumber}</td>
