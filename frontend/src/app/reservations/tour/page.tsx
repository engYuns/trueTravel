...
export default function ReservationsTourPage() {
  ...
  const handleSearch = () => {
    console.log('Searching tour reservations with criteria:', {
      dateFrom,
      dateTo,
      sort,
      transactionType,
  ...
  const handleExport = async (type: 'copy' | 'excel' | 'pdf' | 'print') => {
    ...
    if (type === 'print') {
      printTable('#export-table', 'Reservations - Tour');
      return;
    }
    if (type === 'copy') {
    ...
    if (type === 'excel') {
      await exportTableToExcel('reservations-tour.xlsx', data);
      return;
    }
    await exportTableToPdf('reservations-tour.pdf', 'Reservations - Tour', data);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-6">
          <h2 className="text-3xl font-light text-gray-800">Tour</h2>
        ...
        <div className="bg-white border border-orange-500 rounded-sm overflow-hidden mb-8">
          <div className="bg-orange-500 text-white px-5 py-3 flex items-center justify-between">
            ...
            <button type="button" className="text-white/90 hover:text-white" aria-label="Collapse searching criterias">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-5">
              ...
              <div>
                ...
                >
                  ...
                  <option value="Start / End date">Start / End date</option>
                </select>
              </div>
              ...
              {/* Tour (two-part control as screenshot) */}
              <div className="lg:col-span-2">
              ...
              {/* Tour Type */}
              <div>
                <label className="block text-sm font-medium text-orange-500 mb-2">Tour Type</label>
                <select
                  value={tourType}
              ...
              {/* Buttons column (Search + Excel inside criteria, like screenshot) */}
              <div className="lg:row-span-2 flex flex-col items-end justify-center gap-3">
                <button
                  type="button"
                ...
                <button
                  ...
                  className="w-full lg:w-auto px-8 py-2 border border-blue-500 text-blue-600 rounded-md bg-white hover:bg-blue-50 transition-colors font-medium flex items-center justify-center space-x-2"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
                  </svg>
            ...
            <table id="export-table" className="w-full">
              <thead className="bg-gray-700 text-white">
                <tr>
                  ...
                  <th className="px-4 py-4 text-left font-semibold">Tour Code</th>
                  ...
                  <th className="px-4 py-4 text-left font-semibold">Tour Type</th>
                  <th className="px-4 py-4 text-left font-semibold">Service Name</th>
                  <th className="px-4 py-4 text-left font-semibold">Passenger</th>
                  <th className="px-4 py-4 text-left font-semibold">Room</th>
                  <th className="px-4 py-4 text-left font-semibold">Pax Count</th>
                  <th className="px-4 py-4 text-left font-semibold">Amount</th>
                  <th className="px-4 py-4 text-left font-semibold">Extra Tours</th>
                  <th className="px-4 py-4 text-left font-semibold">Reservation Note</th>
                </tr>
              </thead>
              <tbody>
                {reservations.length === 0 ? (
                  <tr>
                    <td colSpan={15} className="px-6 py-10 text-center text-gray-700">
                  ...
                  reservations.map((row, idx) => (
                    <tr key={idx} className="border-t border-gray-200 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-700">{row.tourCode}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-700">{row.endDate}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.tourType}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.serviceName}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.passenger}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.room}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.paxCount}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.amount}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.extraTours}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.reservationNote}</td>
                    </tr>
                  ))
                )}
              </tbody>
