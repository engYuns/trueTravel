...
export default function ReservationsVisaPage() {
  ...
  const [operationalStatusLeft, setOperationalStatusLeft] = useState('All');
  const [operationalStatusRight, setOperationalStatusRight] = useState('All');
  ...
  const handleSearch = () => {
    console.log('Searching visa reservations with criteria:', {
      dateFrom,
      dateTo,
      sort,
      transactionType,
      ...
      user,
      companyReference,
      operationalStatusLeft,
      operationalStatusRight,
      channelType,
      currentPage,
      itemsPerPage,
    });
  ...
  const handleExport = async (type: 'copy' | 'excel' | 'pdf' | 'print') => {
    ...
    if (type === 'print') {
      printTable('#export-table', 'Reservations - Visa');
      return;
    }
    if (type === 'copy') {
    ...
    if (type === 'excel') {
      await exportTableToExcel('reservations-visa.xlsx', data);
      return;
    }
    await exportTableToPdf('reservations-visa.pdf', 'Reservations - Visa', data);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100">
              ...
              {showAgencyDropdown && (
                <div className="absolute top-full left-0 mt-0 w-56 bg-black rounded-lg shadow-lg py-2 z-[999999] border border-gray-700">
                  ...
                  <a href="/agency/customers" className="block px-4 py-3 text-white hover:bg-gray-800 transition-colors">
                    <div className="flex items-center space-x-3">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 0 1 8 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                      </svg>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-6">
          <h2 className="text-3xl font-light text-gray-800">Visa</h2>
        ...
        <div className="bg-white border border-orange-500 rounded-sm overflow-hidden mb-8">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
              ...
              {/* Operational Status (two selects) */}
              <div className="lg:col-span-2">
                <label className="block text-sm font-medium text-orange-500 mb-2">Operational Status</label>
                <div className="grid grid-cols-2 gap-3">
                  <select
                    value={operationalStatusLeft}
                    onChange={(e) => setOperationalStatusLeft(e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                  >
                    <option value="All">All</option>
                  </select>
                  <select
                    value={operationalStatusRight}
                    onChange={(e) => setOperationalStatusRight(e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                  >
                    <option value="All">All</option>
                  </select>
              ...
              <div className="p-6">
                <div className="border border-gray-200 overflow-hidden">
                  <table className="w-full">
                    ...
                    <tbody>
                      <tr>
                        <td colSpan={2} className="px-4 py-8 text-center text-gray-400"></td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            <table id="export-table" className="w-full">
              <thead className="bg-gray-700 text-white">
                <tr>
                  ...
                  <th className="px-4 py-4 text-left font-semibold">Operational Status</th>
                  <th className="px-4 py-4 text-left font-semibold">Accommodation Date</th>
                  <th className="px-4 py-4 text-left font-semibold">Visa</th>
              ...
              <tbody>
                {reservations.length === 0 ? (
                  <tr>
                    <td colSpan={11} className="px-6 py-10 text-center text-gray-700">
                  ...
                  reservations.map((row, idx) => (
                    <tr key={idx} className="border-t border-gray-200 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-700">{row.operationalStatus}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.accommodationDate}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{row.visa}</td>
