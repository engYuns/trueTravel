...
type RentACarRuleRow = {
  ...
  agency: string;
  reservationDatePickupDate: string;
  dropoffDate: string;
};
...
export default function RentACarPriceRulesPage() {
  ...
  // Table Data
  const [rentACarRules, setRentACarRules] = useState<RentACarRuleRow[]>([]);
  ...
  const filteredRules = useMemo(() => {
    ...
    return rentACarRules.filter((rule) => {
      ...
      if (channelNeedle && channelNeedle !== 'All' && rule.salesChannel !== channelNeedle) return false;
      return true;
    });
  }, [rentACarRules, appliedFilters]);
