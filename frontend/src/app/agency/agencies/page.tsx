...
export default function AgenciesPage() {
  ...
  const [iataType, setIataType] = useState('All');
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setAgencies(agencies.map(a => ({ ...a, selected: checked })));
  };
        ...
        <div className="bg-white rounded-lg shadow-md mb-6">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              {/* Agency Group */}
              <div>
                <label className="block text-sm font-medium text-orange-500 mb-2">Agency Group</label>
                <select
                  value={agencyGroup}
              ...
              {/* IATA Type */}
              <div>
                <label className="block text-sm font-medium text-orange-500 mb-2">IATA Type</label>
                <select
                  value={iataType}
                  onChange={(e) => setIataType(e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-4 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-orange-500"
                >
              ...
              {/* Region */}
              <div>
                <label className="block text-sm font-medium text-orange-500 mb-2">Region</label>
                <input
                  type="text"
                  value={region}
              ...
              {/* Country and City */}
              <div>
                <label className="block text-sm font-medium text-orange-500 mb-2">Country and City</label>
                <select
                  value={countryCity}
        ...
        <div className="flex flex-wrap items-center justify-between mb-4 gap-4">
          <div className="flex items-center space-x-2">
            ...
            <div className="relative">
              ...
              {showColumnMenu && (
                <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg p-4 z-50 min-w-[200px]">
                  {Object.keys(visibleColumns).map(column => (
                    <label key={column} className="flex items-center space-x-2 mb-2 cursor-pointer">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-sm text-gray-700 capitalize">{column.replace(/([A-Z])/g, ' $1').trim()}</span>
                    </label>
                  ))}
                </div>
              )}
            ...
            <button
              ...
              className="bg-white border border-teal-400 text-teal-600 px-4 py-2 rounded-lg hover:bg-teal-50 transition-colors flex items-center space-x-2"
            >
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            ...
            <button
              ...
              className="bg-white border border-yellow-400 text-yellow-600 px-4 py-2 rounded-lg hover:bg-yellow-50 transition-colors flex items-center space-x-2"
            >
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            ...
            <button
              ...
              className="bg-white border border-red-400 text-red-600 px-4 py-2 rounded-lg hover:bg-red-50 transition-colors flex items-center space-x-2"
            >
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="flex items-center space-x-4">
            <span className="text-orange-500 font-medium">Total Recording Number: {agencies.length}</span>
            <div className="flex items-center space-x-2">
            ...
            <table className="w-full">
              <thead className="bg-gray-600 text-white">
                <tr>
                  ...
                  {visibleColumns.status && <th className="px-4 py-3 text-left font-medium">Status</th>}
                  {visibleColumns.code && <th className="px-4 py-3 text-left font-medium">Code</th>}
                  {visibleColumns.accountingCode && <th className="px-4 py-3 text-left font-medium">Accounting Code</th>}
                  {visibleColumns.mainAgency && <th className="px-4 py-3 text-left font-medium">Main Agency</th>}
                  {visibleColumns.agencyName && <th className="px-4 py-3 text-left font-medium">Agency Name</th>}
                  {visibleColumns.invoice && <th className="px-4 py-3 text-left font-medium">Invoice</th>}
                  {visibleColumns.administrator && <th className="px-4 py-3 text-left font-medium">Administrator</th>}
                  {visibleColumns.country && <th className="px-4 py-3 text-left font-medium">Country</th>}
                  {visibleColumns.city && <th className="px-4 py-3 text-left font-medium">City</th>}
                  {visibleColumns.region && <th className="px-4 py-3 text-left font-medium">Region</th>}
                ...
                {agencies.map((agency) => (
                  <tr key={agency.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    {visibleColumns.status && (
                      <td className="px-4 py-3 text-green-600 font-medium">{agency.status}</td>
                    )}
                    {visibleColumns.code && <td className="px-4 py-3 text-gray-900">{agency.code}</td>}
                    {visibleColumns.accountingCode && <td className="px-4 py-3 text-gray-900">{agency.accountingCode}</td>}
                    {visibleColumns.mainAgency && <td className="px-4 py-3 text-gray-900">{agency.mainAgency}</td>}
                    {visibleColumns.agencyName && <td className="px-4 py-3 text-gray-900">{agency.agencyName}</td>}
                    {visibleColumns.invoice && <td className="px-4 py-3 text-gray-900">{agency.invoice}</td>}
                    {visibleColumns.administrator && <td className="px-4 py-3 text-gray-900">{agency.administrator}</td>}
                    {visibleColumns.country && <td className="px-4 py-3 text-gray-900">{agency.country}</td>}
                    {visibleColumns.city && <td className="px-4 py-3 text-gray-900">{agency.city}</td>}
                    {visibleColumns.region && <td className="px-4 py-3 text-gray-900">{agency.region}</td>}
